...
Now using this information (party preference and turnout rate by demographic group) along with state-level demographic data, what I can do (as a side exploratory analysis) is to calculate the expected party preference for each state based solely on its demographics.
...
While an interesting side point, the state bias doesn't actually affect my main analysis in any way. In my code, the `state_bias` variable is used as just a shortcut to perform some computations.^[It's also in my code because I originally did my calculations slightly differently, where the state bias variable would actually affect to the analysis results, albeit to only a small degree.] For my analysis, I simply start with the 2016 election results and directly add the estimated effect from increasing voter turnout. I explain this in more detail in the next section. 
...
For every state, I calculate the difference between the GOP vote percentage based on the expected voter turnout and the GOP vote percentage under a 100% turnout rate (or any other counterfactual turnout rate). I take the difference between these two values and add it to the 2016 election result. 
...
# Calculate state-level party preference in 2016 under 2008 turnout for race
...
4. **Non-voting population**: I assume that the non-voting population votes exactly the same way as the voting population based on their demographics. 
5. **No third party**: I assume that there are only 2 parties, so every party preference metric is calculated as a GOP:Democratic ratio.
